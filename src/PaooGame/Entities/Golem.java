...
/// Clasa Golem reprezintă un inamic care poate fi controlat de AI.
/// Aceasta extinde clasa Enemy și implementează interfața MovableEntity.

public class Golem extends Enemy implements MovableEntity{
    ...
    /// detectie
    private final float Detection_RadiusX = 550;
    private final float Detection_RadiusY = 150;
    private final float Attack_Radius = 100;
    ...
    public Golem(int x, int y) {
        super(x, y, 180, 120, 200);

        this.idleFrames = Assets.golemIdleRight; // dacă e doar un frame
        this.attackFrames = Assets.golemAttackRight;
        this.walkFrames = Assets.golemWalkRight;
        ...
        if (animationTick >= animationSpeed) {
            ...
            if (frameIndex >= maxFrames) {
                ...
                if (currentState == State.ATTACK) {
                    currentState = State.IDLE; // După atac, golemul revine la IDLE
    ...
    if (dx <= Detection_RadiusX && dy <= Detection_RadiusY) { // 🔥 În raza de detecție
        if (dx <= Attack_Radius) { // Dacă este și mai aproape pentru atac
            ...
            if (Attack_Cooldown <= 0) {
                // De exemplu, poți lansa un atac special aici dacă este necesar
                Attack_Cooldown = 60;
            }
        } else {
            // Este în raza de detecție, dar prea departe pentru atac — doar merge spre player
            if (player.getX() < x) {
                x -= speed;
            } else {
                x += speed;
    ...
    private void attack(Player player) {
        ...
        Rectangle attackBox;
        if (facingRight) {
            // Dacă golemul e orientat spre dreapta, pătratul albastru este la dreapta sa
            attackBox = new Rectangle(x + width, y + height / 2 - attackHeight / 2, attackWidth, attackHeight);
        } else {
            // Dacă golemul e orientat spre stânga, pătratul albastru este la stânga sa
    ...
    int attackWidth = 180; // Dimensiunea zonei de atac
    int attackHeight = 64;
